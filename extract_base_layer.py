...
def get_fields_from_bs(bs_object, field_dict):
    ...
    """
    row = {}
    exceptions = ['creator', 'contributor', 'depositor', 'publisher,' 'copyright_status', 'genre', 'type_of_resource']
    ...
    for u in field_dict.keys():
        ...
        for exp in expressions:
            ...
            if u == 'creator' or u == 'contributor' or u == 'depositor' or u == 'publisher':
                results = bs_object.select(exp)
...
def get_name_by_grand_child(bs_object, grand_child_exp, grand_child_element_value, children='namePart' ):
    """
    This function accepts a BeautifulSoup object, find role > roleTerm="creator", "depositor",
    "contributor", or "publisher" and then if it matches, we return all namePart element values, comma separated
    """
